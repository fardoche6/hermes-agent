...
def isolated_kanban_home_with_profiles(monkeypatch):
    ...
    for prof in ("alpha", "beta", "default"):
        ...
        # The dispatcher admits only *configured* profiles (a directory
        # holding a parsable ``config.yaml`` mapping), not bare dirs.
        with open(os.path.join(prof_dir, "config.yaml"), "w", encoding="utf-8") as fh:
            fh.write(f"profile: {prof}\n")
